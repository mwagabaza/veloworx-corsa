  ...
  return (
    <motion.nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? "bg-ink/90 backdrop-blur-md py-4" : "bg-transparent py-8"
      }`}
    >
      <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <button className="magnetic-btn px-6 py-3 hidden md:flex">
          <span>Book Service</span>
        </button>

        <button className="md:hidden text-paper">
          <span className="block w-6 h-[2px] bg-paper mb-1"></span>
          <span className="block w-6 h-[2px] bg-paper mb-1"></span>
          <span className="block w-6 h-[2px] bg-paper"></span>
        </button>
      </div>
    </motion.nav>
  )
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-6 right-6 md:left-12 md:right-12 top-1/2 -translate-y-1/2 flex justify-between items-center z-30 pointer-events-none">
        <button
          onClick={prevSlide}
          className="pointer-events-auto p-3 rounded-full border border-paper/20 bg-ink/40 backdrop-blur-md text-paper/70 hover:text-accent hover:border-accent hover:bg-ink/80 transition-all duration-300 group"
      ...
      <div className="container mx-auto px-6 md:px-12">
        <FadeIn>
          <div className="flex justify-between items-end mb-16 border-b border-ink/20 pb-8">
            ...
            <button className="group flex items-center gap-2 font-display uppercase tracking-widest text-sm hover:text-accent transition-colors">
              View All{" "}
              <ArrowRight
                size={16}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((cat, i) => (
            <FadeIn key={cat.title} delay={i * 0.1}>
              <div className="group relative aspect-[4/3] overflow-hidden bg-ink cursor-pointer">
                <img
                  src={cat.img}
                  alt={cat.title}
        ...
        <FadeIn>
          ...
          <ul className="space-y-4 mb-12">
            {[
              ...
              <li
                key={item}
                className="flex items-center gap-4 text-paper/90 font-sans text-sm uppercase tracking-wide border-b border-paper/10 pb-4"
              >
      ...
      <AnimatePresence>
        {selectedImg && (
          ...
          >
            <button
              onClick={() => setSelectedImg(null)}
              className="absolute top-6 right-6 p-3 text-paper/70 hover:text-accent transition-colors z-10"
